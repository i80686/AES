# http://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197.pdf
class AES:
    def __init__(self):
        self.Nk = 4
        self.Nb = 4
        self.Nr = 10

    def arrays(self, raws):
        Nb = []
        for i in range(4):
            Nb = Nb + [raws[4*0+i], raws[4*1+i], raws[4*2+i], raws[4*3+i]]
        return Nb

    def Inv_arrays(self, raws):
        Inv_raws = []
        for i in range(4):
            Inv_raws = Inv_raws + [raws[4*0+i], raws[4*1+i], raws[4*2+i], raws[4*3+i]]
        return Inv_raws

    def view(self, raws):
        raws = self.Inv_arrays(raws)
        raws = ''.join([x.to_bytes(1, byteorder='big').hex() for x in raws])
        print(raws)

    def AddRoundKey(self, raws, Keys):
        AddRoundKey = []
        for raw, Key in zip(raws, Keys):
            AddRoundKey.append(raw ^ Key)
        return AddRoundKey

    def SubBytes(self, raws):
        S_box = [
            
        ]
        raws_S_box = []
        for raw in raws:
            raws_S_box.append(S_box[raw])
        return raws_S_box

    def ShiftRows(self, raws):
        s10 = raws.pop(4)
        raws.insert(7, s10)
        s2021 = raws[8:10]
        del raws[8:10]
        raws[10:0] = s2021
        s303132 = raws[12:15]
        del raws[12:15]
        raws[13:0] = s303132
        return raws

    def GMUL(self, a, b):      #Russian Peasant Multiplication algorithm
        p = 0
        while a and b:
            if b & 1:    # b%2
                p = p ^ a
            if a & 0x80:   # a=a*x^7(a>0),a >= 2**7(128)
                a = (a << 1) ^ 0x11b # 0x11b = x^8 + x^4 + x^3 + x + 1 (0b100011011)
            else:
                a = a << 1
            b = b >> 1
        return p

    def MixColumns(self, raws):
        for i in range(4):
            raws[0*4+i], \
            raws[1*4+i], \
            raws[2*4+i], \
            raws[3*4+i]  \
            = \
            self.GMUL(0x02, raws[0*4+i]) ^ self.GMUL(0x03, raws[1*4+i]) ^ raws[2*4+i] ^ raws[3*4+i],\
            raws[0*4+i] ^ self.GMUL(0x02, raws[1*4+i]) ^ self.GMUL(0x03, raws[2*4+i]) ^ raws[3*4+i],\
            raws[0*4+i] ^ raws[1*4+i] ^ self.GMUL(0x02, raws[2*4+i]) ^ self.GMUL(0x03, raws[3*4+i]),\
            self.GMUL(0x03, raws[0*4+i]) ^ raws[1*4+i] ^ raws[2*4+i] ^ self.GMUL(0x02, raws[3*4+i])
        return raws

    def RotWord(self, temp):
        b0 = temp.pop(0)
        temp.insert(3, b0)
        return temp

    def SubWord(self, temp):
        temp = self.SubBytes(temp)
        return temp

    def KeyExpansion(self, key):
        i = 0
        w = [[0]]*(self.Nb * (self.Nr + 1))
        Rcon = [[0x01, 0x00, 0x00, 0x00],
                [0x02, 0x00, 0x00, 0x00],
                [0x04, 0x00, 0x00, 0x00],
                [0x08, 0x00, 0x00, 0x00],
                [0x10, 0x00, 0x00, 0x00],
                [0x20, 0x00, 0x00, 0x00],
                [0x40, 0x00, 0x00, 0x00],
                [0x80, 0x00, 0x00, 0x00],
                [0x1B, 0x00, 0x00, 0x00],
                [0x36, 0x00, 0x00, 0x00]
                ]
        while i < self.Nk:
            w[i] = ([key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]])
            i = i + 1

        i = self.Nk

        while i < self.Nb * (self.Nr + 1):
            temp = w[i - 1].copy()
            if i % self.Nk == 0:
                temp = self.SubWord(self.RotWord(temp))
                temp2 = []
                for temp1, Rcon1 in zip(temp, Rcon[(i // self.Nk)-1]):
                    temp2.append(temp1 ^ Rcon1)
                temp = temp2
            elif self.Nk > 6 and i % self.Nk == 4:
                temp = self.SubWord(temp)
            w_temp = []
            for w1, temp1 in zip(w[i-self.Nk], temp):
                w_temp.append(w1 ^ temp1)
            w[i] = w_temp
            i = i + 1
        return w

    def AES(self, Input, Cipher_Key):
        Input = [Input1 for Input1 in Input]
        Cipher_Key = [Cipher_Key1 for Cipher_Key1 in Cipher_Key]
        KeyExpansion = self.KeyExpansion(Cipher_Key)
        keys = []
        for Key_index in range(len(KeyExpansion)//4):
            keys_temp = (KeyExpansion[4*Key_index] + KeyExpansion[4*Key_index+1] + KeyExpansion[4*Key_index+2] + KeyExpansion[4*Key_index+3])
            keys_temp = self.arrays(keys_temp)
            keys.append(keys_temp)
        Input = self.arrays(Input)
        self.view(Input)
        self.view(keys[0])
        Input = self.AddRoundKey(Input,keys[0])
        self.view(Input)
        for index in range(self.Nr-1):
            Input = self.SubBytes(Input)
            self.view(Input)
            self.ShiftRows(Input)
            self.view(Input)
            #print(hex(self.GMUL(0x57, 0x83)))
            self.MixColumns(Input)
            self.view(Input)
            Input = self.AddRoundKey(Input, keys[index+1])
            self.view(Input)
        Input = self.SubBytes(Input)
        self.view(Input)
        Input = self.ShiftRows(Input)
        self.view(Input)
        Input = self.AddRoundKey(Input, keys[-1])
        self.view(Input)
        Input =self.Inv_arrays(Input)
        Input = bytes(Input)
        return Input

Input = bytes.fromhex('00112233445566778899aabbccddeeff')
Cipher_Key = bytes.fromhex('000102030405060708090a0b0c0d0e0f')
Out = AES().AES(Input, Cipher_Key)
print(Out)
